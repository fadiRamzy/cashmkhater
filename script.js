/* ====================================================
   Care with Fady — Smart Arabic Diet Planner
   Fully offline | No backend | Pure JS calculations
   ==================================================== */

// ============================================
//   UPDATE 1 — SMART ARABIC TEXT NORMALIZATION
//   & FOOD EXCLUSION SYSTEM
// ============================================

/**
 * Normalize Arabic text for smart comparison:
 * - Remove "ال" prefix
 * - Unify letter variants (ى→ي, ة→ه, أإآ→ا)
 * - Strip diacritics
...
function normalizeArabic(text) {
  ...
  s = s.replace(/[أإآ]/g, 'ا');                // unify alef variants
  s = s.replace(/ة/g, 'ه');                    // taa marbuta → ha
  s = s.replace(/ى/g, 'ي');                    // alef maqsura → ya
  s = s.replace(/\bال/g, '');                  // remove ال prefix
  s = s.replace(/\s+/g, ' ').trim();
  return s;
}

/**
 * Parse the avoidFoods text into an exclusion list.
 * Splits on comma, Arabic comma, or spaces.
 */
...
/**
 * Returns true if a food name matches any excluded term (partial match).
 */
function isFoodExcluded(foodName, excludeList) {
  if (!excludeList || excludeList.length === 0) return false;
  const normalized = normalizeArabic(foodName);
  return excludeList.some(term => normalized.includes(term) || term.includes(normalized));
}

// ============================================
//   UPDATE 6 — EXPANDED FOOD DATABASE (200+)
// ============================================
// Each item: { name, cal, pro, carb, fat, unitName, unitWeight, unit, portion, budget }
// unitName   = display label (ثمرة، علبة، كوب...)
// unitWeight = grams per that unit (used for quantity display)
// unit       = legacy field kept for compatibility
// Display:   "1 ثمرة (100 جرام)"
...
const FOODS = {
  ...
  proteins: [
    ...
    { name: "دجاج مقلي خفيف",           cal: 230, pro: 25,  carb: 4,   fat: 13,  unitName: "قطعة",    unitWeight: 120, unit: "قطعة",               portion: 1, budget: "mid" },
    { name: "فراخ كاملة مشوية",         cal: 180, pro: 25,  carb: 0,   fat: 9,   unitName: "جرام",    unitWeight: 100, unit: "جم",                 portion: 100, budget: "mid" },
    ...
    { name: "شوربة عدس",                cal: 130, pro: 9,   carb: 22,  fat: 1,   unitName: "طبق",     unitWeight: 300, unit: "طبق كبير",           portion: 1, budget: "low" },
  ...
  fats: [
    ...
    { name: "مايونيز خفيف",             cal: 45,  pro: 0.1, carb: 1,   fat: 4.5, unitName: "ملعقة",   unitWeight: 15,  unit: "ملعقة كبيرة",        portion: 1, budget: "low" },
  ],

  // ======= CATEGORY 6: وجبات مصرية =======
  egyptian: [
  ...
  dairy: [
    ...
    { name: "قشطة بالعسل",             cal: 180, pro: 3,   carb: 20,  fat: 10,  unitName: "طبق",     unitWeight: 80,  unit: "طبق صغير",           portion: 1, budget: "mid" },
    { name: "جرانولا بالزبادي",        cal: 220, pro: 9,   carb: 33,  fat: 7,   unitName: "طبق",     unitWeight: 180, unit: "طبق",                portion: 1, budget: "mid" },
    { name: "لبن زبادي يوناني بالعسل", cal: 160, pro: 14,  carb: 18,  fat: 2,   unitName: "كوب",     unitWeight: 200, unit: "كوب",                portion: 1, budget: "mid" },
  ],

  snacks: [
    { name: "زبادي بالفواكه",           cal: 140, pro: 7,   carb: 24,  fat: 2,   unitName: "كوب",     unitWeight: 200, unit: "كوب",                portion: 1, budget: "low" },
    { name: "عصير برتقال طازج",         cal: 110, pro: 1.7, carb: 26,  fat: 0.3, unitName: "كوب",     unitWeight: 250, unit: "كوب كبير",           portion: 1, budget: "low" },
    { name: "سموذي موز وحليب",          cal: 200, pro: 6,   carb: 38,  fat: 3,   unitName: "كوب",     unitWeight: 300, unit: "كوب كبير",           portion: 1, budget: "low" },
    ...
    { name: "سموذي فراولة وزبادي",      cal: 180, pro: 8,   carb: 32,  fat: 2,   unitName: "كوب",     unitWeight: 300, unit: "كوب كبير",           portion: 1, budget: "mid" },
  ]
};
...
const MEAL_TEMPLATES = {
  breakfast: [
    ...
    { protein: ["بيض مقلي بالزيت", "بيض عيون بالسمن", "بيض أومليت بالخضروات"], carb: ["خبز بلدي", "خبز توست أسمر", "خبز أسمر بلدي"], side: ["خيار وطماطم", "طماطم طازجة"] },
    { protein: ["فول مدمس", "فول مدمس بالزيت والكمون"], carb: ["خبز بلدي", "عيش شمس"], side: ["خيار طازج", "طماطم طازجة"] },
    { protein: ["زبادي يوناني خالي الدسم", "جبنة قريش", "زبادي خالي الدسم"], carb: ["شوفان مطبوخ بالماء", "كورن فليكس بالحليب", "شوفان بالحليب والعسل"], side: ["موز", "فراولة", "تفاح", "كيوي"] },
    { protein: ["بيض مسلوق", "زبادي عادي"], carb: ["خبز توست أسمر", "خبز صاج رقيق"], side: ["خيار طازج", "جرجير طازج"] },
    { protein: ["بيض مخفوق بالحليب", "بيض أومليت بالخضروات"], carb: ["خبز بلدي", "عيش شمس"], side: ["طماطم طازجة", "فلفل أخضر"] },
  ...
  lunch: [
    ...
    { protein: ["فراخ كاملة مشوية", "صدر دجاج مشوي", "مكورة دجاج مشوية"], carb: ["أرز بني مطبوخ", "كسكس مطبوخ", "فريك مطبوخ"], veggie: ["فاصوليا خضراء مسلوقة", "بروكلي مسلوق", "قرنبيط مطبوخ"] },
    ...
    { protein: ["ملوخية بالدجاج", "بامية باللحمة"], carb: ["أرز أبيض مطبوخ", "خبز بلدي"], veggie: ["سلطة خضراء مشكلة", "خيار طازج"] },
    ...
    { protein: ["صدر دجاج مشوي", "دجاج مقلي خفيف"], carb: ["مكرونة قمح كامل مسلوقة", "ذرة مسلوقة"], veggie: ["باذنجان مشوي", "فلفل أحمر", "كوسة مشوية"] },
  ...
  dinner: [
    ...
    { protein: ["جبنة قريش", "زبادي يوناني خالي الدسم", "زبادي خالي الدسم"], carb: ["شوفان مطبوخ بالماء", "خبز توست أسمر"], veggie: ["تفاح", "موز", "كيوي"] },
    ...
    { protein: ["كبدة مشوية", "دجاج مقلي خفيف"], carb: ["خبز بلدي", "ذرة مسلوقة"], veggie: ["كرنب مسلوق", "طماطم طازجة"] },
  ]
};
 ...
 * Format: "1 طبق (200 جرام)" or "150 جرام"
 */
function formatAmount(food, ratio) {
  ...
  if (unitName === 'جرام' || unitName === 'جم') {
    return `${grams} جرام`;
  }

  // Round portion to nearest 0.5
...
function renderResults(u) {
  ...
  if (warnings.length > 0) { warningText.innerHTML = warnings.join('<br>'); warningBox.style.display = 'flex'; }
  ...
  document.getElementById('analysisGrid').innerHTML = `
    ...
    ${u.visceralFat ? `<div class="analysis-card" style="--accent-color:#c0392b"><div class="card-icon">🫀</div><div class="card-value">${u.visceralFat}</div><div class="card-unit">درجة</div><div class="card-label">دهون منطقة البطن (الحشوية)</div></div>` : ''}
  `;
  ...
  // ---- Diet Plan (UPDATE 5 validation inside) ----
  ...
  document.getElementById('recommendationsSection').innerHTML = `
    <div class="recommendations-section">
      ...
      <div class="recs-grid">
        ...
        <div class="rec-card"><div class="rec-icon">🏃</div><div class="rec-title">التمارين المقترحة</div><div class="rec-value" style="font-size:0.9rem">${exerciseRec.type}</div><div class="rec-note">${exerciseRec.duration}</div></div>
...
// ============================================
//   UPDATE 5 — VALIDATION WRAPPER
//   Retry if excluded foods sneak into the plan
// ============================================
function buildAndRenderMealPlan(totalCals, macros, u) {
  const excludeList = parseExcludeList((u.avoidFoods || '') + '،' + (u.allergies || ''));
  ...
  dietSection.innerHTML = `<h3>🍽️ النظام الغذائي اليومي</h3>` + notice + plan.map(meal => renderMealCard(meal)).join('');
}

// ============================================
//   BUILD MEAL PLAN
// ============================================
function buildMealPlan(totalCals, macros, userData, excludeList = []) {
  const mealCals = {
  ...
  // Pick a food from an array, preferring non-excluded ones
  function safeRand(arr) {
    const ok = arr.filter(n => !isFoodExcluded(n, excludeList));
    return randFrom(ok.length > 0 ? ok : arr);
  }

  // Pick a template where every slot has at least one valid food
...
function buildStructuredMeal(name, icon, time, targetCals, foodNames, bgColor) {
  ...
  foodNames.forEach((fname, i) => {
    ...
    items.push(item);
    total.cal  += item.cal; total.pro  += item.pro;
    total.carb += item.carb; total.fat += item.fat;
  });
...
// ============================================
//   INIT — checkbox mutual exclusion
// ============================================
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (noneCheck) {
    noneCheck.addEventListener('change', () => {
      if (noneCheck.checked) allChecks.forEach(c => { if (c !== noneCheck) c.checked = false; });
    });
    allChecks.forEach(c => {
      if (c !== noneCheck) c.addEventListener('change', () => { if (c.checked) noneCheck.checked = false; });
    });
  }
});
